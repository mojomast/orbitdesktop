...
export function checkpointStore(root) {
 ...
 function save(record,label){if(fs.existsSync(path.join(root,'workspace.sqlite')))throw Object.assign(Error('SQLite is authoritative; legacy checkpoint writes are disabled'),{category:'UPGRADE_REQUIRED'});const entry={id:randomUUID(),created:Date.now(),label:String(label||'Checkpoint').slice(0,160),revision:record.revision,state:record.state};const file=path.join(folder(record.id),entry.id+'.json');fs.writeFileSync(file+'.tmp',JSON.stringify(entry),{mode:0o600});fs.renameSync(file+'.tmp',file);return entry;}
