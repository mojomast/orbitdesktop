...
export function createWorkbench({store,token,port,devOrigins,reply,now=Date.now,services={}}){
  ...
  async function dispatch(body){
    ...
    if(body.action==='list'){
      const projects=records.list(body.workspace_id);
      return {projects:projects.map(publicProject),bindings:projects.filter(project=>project.active!==false).flatMap(project=>records.bindings(body.workspace_id,project.id)),revision:workspace.revision,surfaces:panes(workspace.state)};
    }
