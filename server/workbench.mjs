...
export function createWorkbench({store,token,port,devOrigins,reply,now=Date.now,services={}}){
  ...
  async function dispatch(body){
    ...
    if(body.action==='list'){
      ...
      return {projects:projects.map(publicProject),bindings:projects.flatMap(project=>records.bindings(body.workspace_id,project.id)),revision:workspace.revision,surfaces:panes(workspace.state)};
    }
