...
export async function pinnedReleaseIdentity({env=process.env,sourceRoot=fileURLToPath(new URL('../',import.meta.url))}={}){
  ...
  const {verifyRelease,checkCompatibility,currentSchemaVersion}=await import('../scripts/release_launch.mjs');
  const {assertSeparatedRoots}=await import('../scripts/release_roots.mjs');
