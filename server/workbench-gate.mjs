...
export function createWorkbenchGate({legacySnapshot}={}){
  const active=new Map(),quarantines=new Set();let legacy=legacySnapshot??(()=>({enabled:false,active:0,uncertain:0,pending:0}));
  ...
  return {
    ...
    legacyStatus,
    busy(kind){return quarantines.size>0||active.has(lane(kind))||(kind!=='legacy-agent'&&legacyBlocked());},
    ...
    claim(kind,id){
      ...
      const key=lane(kind);
      if(quarantines.size>0||active.has(key)||(kind!=='legacy-agent'&&legacyBlocked())||(kind==='legacy-agent'&&active.has('job')))throw wbError('busy');
    ...
    status(){return {agent:active.has('agent'),job:active.has('job'),legacy:legacyStatus(),...(quarantines.size?{quarantines:[...quarantines]}:{})};},
  };
}
