...
spec=importlib.util.spec_from_file_location('workshop',R/'extensions/cocs-workshop/main.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
class Tests(unittest.TestCase):
 ...
 def test_publish_contract_mock_external_only(self):
  ...
  self.s.update(validation_policy=m.POLICY,validated_digest=self.s['digest'])
  with patch.object(m,'cmd',command),patch.object(m,'compatibility',return_value='Unit-test stub; real browser probe tested separately'):
 ...
 def test_old_ready_draft_is_blocked(self):
  with self.assertRaisesRegex(ValueError,'predates'):m.publish(self.s,{'digest':self.s['digest'],'title':'Old validation','confirm':True})
 def test_map_gate(self):
  self.s['asset']='map:blood-gulch'
  with self.assertRaisesRegex(ValueError,'traversal'):m.compatibility(self.s,['game/maps.mjs'])
 def test_publication_rechecks_compatibility(self):
  self.s.update(validation_policy=m.POLICY,validated_digest=self.s['digest'])
  with patch.object(m,'compatibility',side_effect=ValueError('broken contract')):
   with self.assertRaisesRegex(ValueError,'broken contract'):m.publish(self.s,{'digest':self.s['digest'],'title':'Reject','confirm':True})
