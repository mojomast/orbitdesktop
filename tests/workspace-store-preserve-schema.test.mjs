...
test('normal restore of the same backup migrates the copy to schema 6', async (t) => {
  ...
  assert.equal(rawVersion(path.join(destination, 'workspace.sqlite')), 6);
  assert.equal(sha256(backup), before, 'source backup must remain byte-for-byte unchanged');
  const reopened = new SqliteWorkspaceStore(destination);
  try {
    assert.equal(reopened.diagnostics().schema_version, 6);
...
test('preserve-schema reports schema 6 for a schema-6 backup without tampering', async (t) => {
  ...
  const result = run(['restore', '--runtime', destination, '--source', backup, '--confirm-stopped', '--preserve-schema']);
  assert.deepEqual(result, { restored: true, preserved: true, schema_version: 6 });
  assert.equal(rawVersion(path.join(destination, 'workspace.sqlite')), 6);
});
