...
test('schema 4 upgrades additively and backup preserve-schema can retain the old binary format',async t=>{
  ...
  const migrated=new SqliteWorkspaceStore(path.join(root,'restored'));try{assert.equal(migrated.db.pragma('user_version',{simple:true}),8);assert.equal(migrated.db.prepare('SELECT count(*) AS n FROM wb_projects').get().n,0);}finally{migrated.close();}
});
