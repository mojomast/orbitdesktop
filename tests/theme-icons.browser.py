  ...
  with sync_playwright() as p:
   ...
   for name,style in [('Windows XP','xp'),('Classic 95','classic'),('Paper Studio','paper'),('Cyberpunk','cyberpunk'),('Aurora Glass','aurora'),('Phosphor','phosphor'),('Blueprint','blueprint'),('Pop Art','pop'),('Ocean','ocean'),('Forest','forest'),('Plum','plum'),('Ember','ember'),('Midnight','midnight')]:
    ...
    for role in ['folder','chat','terminal','browser','document','grid','image','key','app']:
     assert page.request.get(f'http://127.0.0.1:4398/icons/{style}/{role}.svg').ok
    assert '/icons/'+style+'/' in page.locator('.start-logo').evaluate('e=>getComputedStyle(e,"::before").backgroundImage')
