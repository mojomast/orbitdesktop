...
export function mountWorkbenchReviewView(body:HTMLElement,args:Args):{refresh():Promise<void>;dispose():void}{
  ...
  async function renderSelected(ticket:number){
    ...
    const left=el('section','workbench-review-diff');left.setAttribute('aria-label','Exact reviewed candidate diff');left.append(el('h4','','Exact candidate diff'),textField('Candidate',`${candidate.id} · generation ${candidate.generation}`),textField('SHA-256',candidate.hash),textField('Source snapshot',patch.source?.manifest_hash),el('pre','workbench-review-diff-text',String(patch.patch_text??'No textual diff is available.')));
    const right=el('section','workbench-review-checks');right.setAttribute('aria-label','Frozen required checks and review evidence');right.append(el('h4','','Required checks and evidence'),textField('Acceptance revision',task?.acceptance_digest),textField('Complete for this exact candidate',complete?'Yes':'No — stale, incomplete, or unavailable'),textField('Verified patch-export support',artifactChecksSupported?'Available after explicit artifact-check confirmation':'Unsupported for profile-bound checks; no profile equivalence is inferred'));
    for(const item of checks){const check=el('article','workbench-review-check');check.append(el('h5','',item.required.definition_id),textField('Profile',item.required.execution_profile_id??'Default trusted check profile'),textField('Result',item.evidence?`${item.evidence.evidence.verdict} · job ${item.evidence.job.id} · evidence ${item.evidence.evidence.id}`:'No complete exact pass for this frozen required check'));if(item.evidence){const details=el('details'),summary=el('summary','','Show check provenance');const raw=el('pre');raw.textContent=JSON.stringify(item.evidence.evidence.provenance??null,null,2);details.append(summary,raw);check.append(details);}right.append(check);}
    ...
    view.replaceChildren(left,right,explanation,human);
