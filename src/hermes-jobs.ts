...
export function showHermesJobs(api: (body: Record<string, unknown>) => Promise<any>) {
 ...
 dialog.append(el('h2','','Hermes scheduled tasks'),button('Close','Close scheduled tasks',()=>dialog.close()),el('p','','Read-only view of this Hermes profile. Tasks execute on the gateway independently of Orbit. Times are shown as returned by Hermes. Up to 200 tasks; prompts and delivery details are not exposed.'),search,button('Refresh','Refresh scheduled tasks',()=>{void refresh();}),status,list);
