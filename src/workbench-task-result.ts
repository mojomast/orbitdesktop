...
export function mountWorkbenchTaskResult(args:Args):{refresh():Promise<void>;dispose():void}{
  ...
  const provenance=(value:Json)=>{
    ...
    const initiatorLabel=initiator.kind==='native_agent'?`Supervised native worker (attempt ${initiator.attempt_id}; grant ${initiator.grant_id}; run ${initiator.run_id})`:initiator.kind==='owner_action'?'Owner action':'Historical origin unknown';
    const authorizerLabel=authorizer.kind==='owner_grant'?`Owner-approved native grant ${authorizer.grant_id} (authority generation ${authorizer.authority_generation})`:authorizer.kind==='owner_approval'?`Owner approval preview ${authorizer.preview_id}`:'Historical authorization unknown';
  ...
  async function refresh(){
    ...
    try{
      ...
      if(result){const option=el('option','',`${result.id} · ${result.availability}${result.unavailable_reason?` (${result.unavailable_reason})`:''}`);option.value=result.id;results.append(option);selectedResult=result.id;render(result,response.grant);}
  ...
  function render(result:Json,grant?:Json){
  ...
  async function retryPersistence(result:Json,expected_digest:string){
    if(busy||disposed)return;busy=true;status.textContent='Retrying durable result finalization only…';error.textContent='';
    try{const response=await api('native',{action:'result_retry',result_id:result.id,expected_digest});if(disposed)return;status.textContent='Result receipt finalized without rerunning Hermes.';render(response.result,{status:'finalized'});args.onChanged?.();}
    catch(reason){if(!disposed){error.textContent=`Result persistence is still pending: ${reason instanceof Error?reason.message:'unavailable'}. No Hermes execution was retried.`;status.textContent='Result persistence pending.';}}
    finally{busy=false;}
  }
