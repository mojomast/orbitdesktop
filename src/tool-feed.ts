...
export function watchToolFeed(id: string, state: () => {session:string;run?:string}, token: () => string) {
  let abort: AbortController | undefined, current = '', stopped = false, retry = 0;
  const seen = new Set<string>();
  async function tick() {
    const s = state();
    if (!sinks.size || !s.run || !token()) {
    ...
    sources.set(id,'Connecting…'); broadcast();
    try {
      ...
      if(!controller.signal.aborted) sources.set(id,'Stream ended · waiting for next request');
    } catch {if(!controller.signal.aborted) sources.set(id,'Disconnected · retrying (events may be missed)');}
