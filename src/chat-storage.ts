...
export type ChatState = { session: string; messages: Message[]; run?: string };
...
export function archiveChat(list: ChatState[], state: ChatState): ChatState[] {
  ...
  return [{ session: state.session, messages: state.messages }, ...list.filter(x => x.session !== state.session)].slice(0, 10);
}
