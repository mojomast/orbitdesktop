...
try {
  ...
  if(!['migrate','diagnose','backup','export-legacy','restore'].includes(action)||!options.runtime)throw Error('Usage: workspace_store.mjs migrate|diagnose|backup|export-legacy|restore --runtime PATH [--destination PATH|--source PATH] [--confirm-stopped]');
  ...
  if(action==='restore') {
    ...
    try {
      ...
      if(options.source!==options.source.trim()||source.pragma('quick_check',{simple:true})!=='ok'||source.pragma('user_version',{simple:true})!==1||source.prepare("SELECT value FROM store_metadata WHERE key='bootstrap_complete'").get()?.value!=='1')throw Error('BACKUP_INVALID');
