...
function backupVersion(db) {
  try {
    ...
    if(version>8)throw Object.assign(Error('UPGRADE_REQUIRED'),{category:'UPGRADE_REQUIRED'});
    if(![1,2,3,4,5,6,7,8].includes(version)||db.pragma('quick_check',{simple:true})!=='ok'||db.prepare("SELECT value FROM store_metadata WHERE key='bootstrap_complete'").get()?.value!=='1')throw backupInvalid();
    return version;
  } catch(error) {
