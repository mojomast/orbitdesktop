...
Read `docs/ONBOARDING.md` for the first-run desktop tour, also available through Start → Getting started. It explains controls, 2D/3D layouts, agent customization and app-building requests with copy-only examples; it never submits messages or changes the layout. Completion is browser-local. Preserve its safety distinctions: named layout collections are browser-local, generated plugins have no privileged host bridge, and checkpoints do not undo shell/file/external effects. New frontend loads show the tour once, including existing users; Escape or Skip dismisses it.

## Orbit secret manager

The owner vault is an independent trusted service at https://kimi.tailec998.ts.net:4364/; read `docs/SECRETS.md`. Metadata-only listing: `python3 scripts/orbit_secrets.py list`. Store through the hidden prompt or private stdin, never literal secrets in shell arguments. Run authorized consumers with `python3 scripts/orbit_secrets.py run MIMO_API_KEY -- COMMAND` to inject selected variables. Do not print values or place them in apps, context, checkpoints, logs, or configuration. Adding a key does not enable a provider or authorize external transmission. Values are encrypted with a host-local key; the owner account/root remains trusted. Vault changes are outside workspace rollback. The UI requires the owner's Tailscale identity. The extension runner does not supervise across reboot.
...
Read `docs/SPATIAL_SCENE.md` for the free-placement 3D renderer, navigation, tiling, surface-resolution controls and deployment boundary. The new frontend needs one initial load; subsequent edits and geometry-only remote synchronization are live. Optional `Monitor.spatial` and `Workspace.spatialCamera` persist without changing 2D frames or pane IDs. The UI was tested against the live server in isolated workspaces. New source-level `arrange_spatial`, `set_spatial_camera`, and `update_window.spatial` controller support requires a controlled backend reload; do not assume those operations are active on a process started before this upgrade. No live terminals were restarted during the frontend release. Workspace checkpoints do not roll back core code.

## Orbit menu (workspace controls)

Desktop controls live in the **orbit menu**, a popout drawer opened from the ◉ orbit logo in the top-left corner (`Open orbit menu` button, drawer id `orbit-menu`). It holds the workspace view switcher (Windows, Spatial, Focus), Themes, Show panel, Full viewport, Wallpaper, Transparency, Global transparency, and the Hermes shortcuts (New chat, Tools and conversations, Shared Chromium, Automated build queue, Jev quick actions, Workspace plugins, Workspace checkpoints, Skills and tools, Apps and outputs, Scheduled tasks). A footer button moves those same controls onto the top toolbar as compact pills and remembers the choice in localStorage. The drawer, its toolbar and the logo are styled by the active theme (see `docs/THEMES.md`). Prefer describing and changing controls this way when guiding the owner; the underlying operations are unchanged.

Live browser tests open these controls through the menu. `tests/orbit_menu.py` exposes `open_menu`, `menu(page, label)` and `close_menu`, and dismisses the first-run tour first because that modal dialog otherwise intercepts the click on the logo on a fresh browser profile.

## Viewport control and capability boundaries

Full viewport is now checkpointed workspace state: use `{"action":"patch_appearance","patch":{"fullViewport":true}}` (false restores controls). Read the current revision first, apply through the scoped controller with base_revision, then check browser_applied and inspect the browser if claiming visibility. This preserves other appearance values. It hides header, sidebar and navigation without changing sidebarHidden; exiting restores the previous sidebar preference. Full viewport keeps a compact orbit logo (which still opens the menu) and a small visible exit control, so Ctrl+Alt+F is not the only way back; manual changes synchronize back to workspace state. Old checkpoints without this field restore normal mode. A client must load the new frontend once; subsequent changes are live.
